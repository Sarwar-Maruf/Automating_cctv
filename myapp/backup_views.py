                ...
                if 'human' in a:
                    ...
                    if len(faces) + len(bodies) > 0:
                        if detection:
                            ...
                            out = cv2.VideoWriter(
                                f"Human_being_Found/{current_time}.mp4", fourcc, 20, frame_size)
                    ...
                    def countFingers(image, results, draw=True, display=True):
                        ...
                        if draw:
                            ...
                            cv2.putText(output_image, " Location: Sukrabad", (10, 25),cv2.FONT_HERSHEY_COMPLEX, 1, (20,255,155), 2)
                            # cv2.putText(output_image, str(sum(count.values())), (width//2-150,240), cv2.FONT_HERSHEY_SIMPLEX,
                            #             8.9, (20,255,155), 10, 10)
                    ...
                    camera_video = cv2.VideoCapture("http://192.168.1.102:8080/video")
                    ...
                    while camera_video.isOpened() and 'emergency' in a:
                        ...
                        print(count)
                        if count == 10 and ((time.time()-t)>=10 or time_flag ==True) :
                            time_flag= False
